...
//        SocketAddress socketAddress =address != null ? new InetSocketAddress(address, port) : null;
    ...
    /**
     * stream=true：表示stream socket 流套接字
     * 使用TCP协议，稳定可靠，占用更多资源
     * <p>
     * stream=false：表示datagram socket 数据报套接字
     * 使用UDP协议，不稳定，占用较少资源
     * <p>
     * 默认TCP
     *
     * @param address   socket套接字地址
     * @param localAddr 本地地址
     * @param stream    是否stream
     ...
     * <p>
     * Nagle算法开启
     * 优点：网络资源占用减少，算法会自动合并小数据包，等到达到一定大小（MSS）后，才会和服务端交互
     * 缺点：实时响应度降低
     *
     ...
     * specific.
     * <p>
     * 默认情况下，我们调用Socket.close()方法会直接返回
     * 当on=true，且linger>0时候
     * 调用Socket.close()方法的线程会被阻塞，等待通信双方发送数据
     * 如果时间过了，还未结束，将发送TCP RST强制关闭TCP
     *
     *
     * <p>
    ...
    public void setSoLinger(boolean on, int linger) throws SocketException {
        if (isClosed())
            throw new SocketException("Socket is closed");
        if (!on) {
            getImpl().setOption(SocketOptions.SO_LINGER, false);
        } else {
            if (linger < 0) {
     ...
     * <p>
     * 表示设置接收端的缓冲区的大小，单位是字节
     * 在套接字建立连接之后，我们可以随意修改窗口大小，但是当窗口大小大于64k时，需要注意：
     ...
     * <p>
     * 用来探测服务端的套接字是否还是存活状态，默认设置是false
     * 如果开启，TCP 自动触发功能
     * 如果两小时内，客户端和服务端的套接字之间没有任何通信，TCP会自动发送keepalive探测给对方
     * 对方必须响应这个探测（假设是客户端发送给服务端），预测有三种情况：
     * 1.服务端使用预期的ACK回复，说明一切正常；
     * 2.服务端回复RST，表示服务端处于死机或者重启状态，终止连接；
     ...
     * 套接字关闭后，close调用
     * 会等待一段时间，才会彻底关闭
     * 如果在等待的时间内，又有新的连接绑定到同样的地址和端口
     * 当SO_REUSEADDR=true时，可以绑定成功
     *
